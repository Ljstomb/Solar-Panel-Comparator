...
const SolarCalculator = () => {
  ...
  const requiredSystemSizeKW = (dailyUsage * 1000) / (sunlightHours * 1000);

  const results = panelTypes.map((panel) => {
    const panelOutput = 300;
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '1rem' }}>
      ...
      <hr />
      {results.map((res, idx) => (
        <div key={idx} style={{ border: '1px solid #ccc', padding: '1rem', marginTop: '1rem' }}>
          <h2>{res.name}</h2>
